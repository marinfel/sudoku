package main.java.com.sudoku.data.factory;
...
public class GridFactory2 {
    ...
    public static Grid generateRandomGrid(int nbEmptyCells){        
        ...
        if (solver.solver.findSolution()) {
        	...
        	Grid g2 = generateGrid(solver);
        	do{
        		for (byte i = 0; i < n; i++) {
            		for (byte j = 0; j < n; j++) {
            			if(g2.getCell(i,j) instanceof FixedCell){
            				g.setFixedCell(i,j,((FixedCell)g2.getCell(i,j)).getValue());
            			} else {
            				g.setEmptyCell(i,j);
            			}
            		}
	        	...
	        	for(int i = 0; i < nbEmptyCells; i++){
	        		...
	        		while((g.getCell(x,y) instanceof EmptyCell));
	        		g.setEmptyCell(x,y);
	        		/*g.setEmptyCell((byte)((n-1)-x),y);
	        		g.setEmptyCell(x,(byte)((n-1)-y));
	        		g.setEmptyCell((byte)((n-1)-x),(byte)((n-1)-y));*/
	            }
	        	solver = generateSolver(g);
        	}
        	while (!(solver.solver.findSolution() && !solver.solver.nextSolution()));
        } else {
        	g = null;
        }
        return g;
